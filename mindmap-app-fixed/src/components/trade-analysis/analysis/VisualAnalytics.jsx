...
import { Card, Grid, HBar, SectionHeader, CardTitle, EmptyHint } from './primitives'
        ...
        <Card delay={0.08}>
          ...
          {age.total === 0 ? (
            ...
            <div className="flex flex-col gap-2">
              <div className="flex h-3 overflow-hidden rounded-full" style={{ backgroundColor: 'var(--ta-bg)' }}>
                {age.fresh > 0 && <div style={{ width: `${(age.fresh / age.total) * 100}%`, backgroundColor: AGE_COLORS.fresh }} />}
                {age.mid > 0 && <div style={{ width: `${(age.mid / age.total) * 100}%`, backgroundColor: AGE_COLORS.mid }} />}
                {age.old > 0 && <div style={{ width: `${(age.old / age.total) * 100}%`, backgroundColor: AGE_COLORS.old }} />}
              </div>
              <div className="flex justify-between text-[8px]" style={{ color: 'var(--ta-slate)' }}>
                <span className="flex items-center gap-1"><i className="h-1.5 w-1.5 rounded-full" style={{ backgroundColor: AGE_COLORS.fresh, display: 'inline-block' }} />0–7d ({age.fresh})</span>
                <span className="flex items-center gap-1"><i className="h-1.5 w-1.5 rounded-full" style={{ backgroundColor: AGE_COLORS.mid, display: 'inline-block' }} />7–30d ({age.mid})</span>
                <span className="flex items-center gap-1"><i className="h-1.5 w-1.5 rounded-full" style={{ backgroundColor: AGE_COLORS.old, display: 'inline-block' }} />30+d ({age.old})</span>
              </div>
            </div>
          )}
        </Card>
