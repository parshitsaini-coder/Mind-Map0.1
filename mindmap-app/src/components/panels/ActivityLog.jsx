    ...
    <div className="flex flex-col gap-2">
      <div className="flex gap-1">
        <button
          ...
          className={`rounded-md border px-2 py-1 text-[10px] ${tab === 'activity' ? 'border-[var(--color-accent)] bg-[var(--color-accent)]/30' : 'border-[var(--color-sage)]'}`}
        >
          Activity
        </button>
        <button
          onClick={() => setTab('versions')}
          className={`rounded-md border px-2 py-1 text-[10px] ${tab === 'versions' ? 'border-[var(--color-accent)] bg-[var(--color-accent)]/30' : 'border-[var(--color-sage)]'}`}
        >
          Versions
        </button>
      </div>
      ...
      {tab === 'activity' ? (
        <ul className="flex max-h-64 flex-col gap-1 overflow-y-auto">
          {activityLog.length === 0 && <p className="text-[10px] text-[var(--color-slate)]">No activity yet.</p>}
          <AnimatePresence initial={false}>
            {activityLog.map((a) => (
              ...
              >
                <Clock size={10} className="mt-0.5 shrink-0 text-[var(--color-slate)]" />
                <span className="flex-1">{a.message}</span>
                <span className="shrink-0 text-[9px] text-[var(--color-slate)]">{timeAgo(a.ts)}</span>
              </motion.li>
            ))}
          </AnimatePresence>
        </ul>
        ...
        <div className="flex flex-col gap-1.5">
          <motion.button
            ...
            whileTap={{ scale: 0.97 }}
            onClick={handleSave}
            className="flex w-fit items-center gap-1 rounded-md border border-[var(--color-slate)] px-2 py-1 text-[10px] hover:bg-[var(--color-sage)]/30"
          >
            <Save size={10} /> Save snapshot now
          </motion.button>
          <ul className="flex max-h-56 flex-col gap-1 overflow-y-auto">
            {snapshots.length === 0 && <p className="text-[10px] text-[var(--color-slate)]">No snapshots yet.</p>}
            <AnimatePresence initial={false}>
              {snapshots.map((snap) => (
                <motion.li
                  key={snap.id}
                ...
                >
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-[9px] text-[var(--color-slate)]">{timeAgo(snap.timestamp)}</p>
                  </div>
